...
static ramblock_t *ls_ram_descriptor = NULL;
static rangeblock_t *ls_range_descriptor = NULL;
static int ls_ramblockcnt = 0;
static const int ls_rangeblockcnt = SPIRAM_BANKSWITCH_RESERVE;
...
static portMUX_TYPE lspinlock = portMUX_INITIALIZER_UNLOCKED;

static inline int lramblock_idx_valid(int ramblock_idx)
{
    return (ramblock_idx >= 0 && ramblock_idx < ls_ramblockcnt);
}

static inline int lrangeblock_idx_valid(int rangeblock_idx)
{
    return (rangeblock_idx >= 0 && rangeblock_idx < ls_rangeblockcnt);
}

static void lset_bank(int virt_bank, int phys_bank, int ct)
{
    int r;
...
size_t lesp_himem_get_free_size()
{
    size_t ret=0;
    for (int i = 0; i < ls_ramblockcnt; i++) {
        if (!ls_ram_descriptor[i].is_alloced) ret+=CACHE_BLOCKSIZE;
    }
    return ret;
}

size_t lesp_himem_reserved_area_size() {
    return CACHE_BLOCKSIZE * SPIRAM_BANKSWITCH_RESERVE;
}


void lesp_himem_init()
{
    if (SPIRAM_BANKSWITCH_RESERVE == 0) return;
    int maxram=esp_spiram_get_size();
    //catch double init
    HIMEM_CHECK(ls_ram_descriptor != NULL, "already initialized", ); //Looks weird; last arg is empty so it expands to 'return ;'
    HIMEM_CHECK(ls_range_descriptor != NULL, "already initialized", );
    ...
    ls_ramblockcnt = ((paddr_end - paddr_start) / CACHE_BLOCKSIZE);
    //Allocate data structures
    ls_ram_descriptor = calloc(sizeof(ramblock_t), ls_ramblockcnt);
    ls_range_descriptor = calloc(sizeof(rangeblock_t), SPIRAM_BANKSWITCH_RESERVE);
    if (ls_ram_descriptor == NULL || ls_range_descriptor == NULL) {
        ESP_EARLY_LOGE(TAG, "Cannot allocate memory for meta info. Not initializing!");
        free(ls_ram_descriptor);
        free(ls_range_descriptor);
        return;
    }
...
static bool lallocate_blocks(int count, uint16_t *blocks_out)
{
    int n = 0;
    for (int i = 0; i < ls_ramblockcnt && n != count; i++) {
        if (!ls_ram_descriptor[i].is_alloced) {
            blocks_out[n] = i;
            n++;
        }
    }
    if (n == count) {
        ...
        for (int i = 0; i < count; i++) {
            ls_ram_descriptor[blocks_out[i]].is_alloced = true;
            assert(ls_ram_descriptor[blocks_out[i]].is_mapped == false);
        }
        return true;
    } else {
...
esp_err_t lesp_himem_alloc(size_t size, esp_himem_handle_t *handle_out)
{
    ...
    portENTER_CRITICAL(&lspinlock);
    int ok = lallocate_blocks(blocks, r->block);
    portEXIT_CRITICAL(&lspinlock);
    if (!ok) {
        goto nomem;
    }
    r->block_ct = blocks;
...
esp_err_t lesp_himem_free(esp_himem_handle_t handle)
{
    //Check if any of the blocks is still mapped; fail if this is the case.
    for (int i = 0; i < handle->block_ct; i++) {
        assert(lramblock_idx_valid(handle->block[i]));
        HIMEM_CHECK(ls_ram_descriptor[handle->block[i]].is_mapped, "block in range still mapped", ESP_ERR_INVALID_ARG);
    }
    //Mark blocks as free
    portENTER_CRITICAL(&lspinlock);
    for (int i = 0; i < handle->block_ct; i++) {
        ls_ram_descriptor[handle->block[i]].is_alloced = false;
    }
    portEXIT_CRITICAL(&lspinlock);
...
esp_err_t lesp_himem_alloc_map_range(size_t size, esp_himem_rangehandle_t *handle_out)
{
    HIMEM_CHECK(ls_ram_descriptor == NULL, "Himem not available!", ESP_ERR_INVALID_STATE);
    ...
    int start_free = 0;
    portENTER_CRITICAL(&lspinlock);
    for (int i = 0; i < ls_rangeblockcnt; i++) {
        if (ls_range_descriptor[i].is_alloced) {
    ...
    for (int i = 0; i < blocks; i++) {
        ls_range_descriptor[r->block_start + i].is_alloced = 1;
    }
    portEXIT_CRITICAL(&lspinlock);
    //All done.
    *handle_out = r;
...
esp_err_t lesp_himem_free_map_range(esp_himem_rangehandle_t handle)
{
    //Check if any of the blocks in the range have a mapping
    for (int i = 0; i < handle->block_ct; i++) {
        assert(lrangeblock_idx_valid(handle->block_start + i));
        assert(ls_range_descriptor[i + handle->block_start].is_alloced == 1); //should be, if handle is valid
        HIMEM_CHECK(ls_range_descriptor[i + handle->block_start].is_mapped, "memory still mapped to range", ESP_ERR_INVALID_ARG);
    }
    //We should be good to free this. Mark blocks as free.
    portENTER_CRITICAL(&lspinlock);
    for (int i = 0; i < handle->block_ct; i++) {
        ls_range_descriptor[i + handle->block_start].is_alloced = 0;
    }
    portEXIT_CRITICAL(&lspinlock);
    free(handle);
    return ESP_OK;
...
esp_err_t lesp_himem_map(esp_himem_handle_t handle, esp_himem_rangehandle_t range, size_t ram_offset, size_t range_offset, size_t len, int flags, void **out_ptr)
{
    ...
    for (int i = 0; i < blockcount; i++) {
        HIMEM_CHECK(ls_ram_descriptor[handle->block[i + ram_block]].is_mapped, "ram already mapped", ESP_ERR_INVALID_STATE);
        HIMEM_CHECK(ls_range_descriptor[range->block_start + i + range_block].is_mapped, "range already mapped", ESP_ERR_INVALID_STATE);
    }

    //Map and mark as mapped
    portENTER_CRITICAL(&lspinlock);
    for (int i = 0; i < blockcount; i++) {
        assert(lramblock_idx_valid(handle->block[i + ram_block]));
        ls_ram_descriptor[handle->block[i + ram_block]].is_mapped = 1;
        ls_range_descriptor[range->block_start + i + range_block].is_mapped = 1;
        ls_range_descriptor[range->block_start + i + range_block].ram_block = handle->block[i + ram_block];
    }
    portEXIT_CRITICAL(&lspinlock);
    for (int i = 0; i < blockcount; i++) {
        lset_bank(VIRT_HIMEM_RANGE_BLOCKSTART + range->block_start + i + range_block, handle->block[i + ram_block] + PHYS_HIMEM_BLOCKSTART, 1);
    }
...
esp_err_t lesp_himem_unmap(esp_himem_rangehandle_t range, void *ptr, size_t len)
{
    ...
    portENTER_CRITICAL(&lspinlock);
    for (int i = 0; i < blockcount; i++) {
        int ramblock = ls_range_descriptor[range->block_start + i + range_block].ram_block;
        assert(lramblock_idx_valid(ramblock));
        ls_ram_descriptor[ramblock].is_mapped = 0;
        ls_range_descriptor[range->block_start + i + range_block].is_mapped = 0;
    }
